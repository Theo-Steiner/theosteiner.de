...
export const ogEntries: OgEntry[] = [
	...
	{ kind: 'page', slug: 'about', title: 'About me.' },
	{ kind: 'page', slug: 'contents', title: 'Everything I’ve made.' },
	...posts.map(({ slug, title }) => ({ kind: 'blog' as const, slug, title })),
	...talkEntries.map(({ slug, title }) => ({ kind: 'talk' as const, slug, title })),
	...podcastEntries.map(({ slug, title }) => ({ kind: 'podcast' as const, slug, title }))
];
...
function japaneseFontSlices(): FontSlice[] {
	...
	return [...css.matchAll(/src: url\(\.\/files\/([^)]+)\)[\s\S]*?unicode-range:\s*([^;]+);/g)].map(
		([, file, unicodeRanges]) => ({
...
function vectorText(
	...
	for (const { font, text } of runs) {
		...
		for (let index = 0; index < layout.glyphs.length; index += 1) {
			...
			if (path) {
				paths.push(
					`<path d="${path}" transform="translate(${cursor + position.xOffset * scale} ${baseline - position.yOffset * scale}) scale(${scale} ${-scale})" fill="#21201c"/>`
				);
			}
